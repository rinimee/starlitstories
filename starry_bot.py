    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                contents = []
                if image_file:
                    # 1. RESET IMAGE FILE CURSOR BEFORE READING
                    image_file.seek(0)
                    file_bytes = image_file.read()
                    if file_bytes:
                        contents.append(types.Part.from_bytes(data=file_bytes, mime_type=image_file.mimetype))
                ...
                # 2. WAIT A MOMENT TO RESTORE BUFFER HEALTH
                time.sleep(1)
                continue
